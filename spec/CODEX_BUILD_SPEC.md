...
- 프론트 상태관리:
  - 전역 상태: `zustand`
  - 원칙: 전역 공유가 필요한 상태에만 사용하고, 폼 입력/일회성 UI 상태는 로컬 상태 유지
...
### 14.4 zustand 사용 규칙 (안티패턴 방지)
- 아래 조건을 2개 이상 만족할 때만 전역 상태로 승격한다.
  - 2개 이상의 위젯/섹션에서 동시에 구독이 필요
  - 페이지 이동/재마운트 시 유지가 필요한 세션 상태
  - prop drilling이 복잡도를 유의미하게 증가시키는 상태
- 전역으로 올리면 안 되는 상태:
  - 단일 컴포넌트에서만 쓰는 입력값
  - 서버 캐시가 더 적합한 비동기 데이터(이 경우 별도 서버 상태 전략 사용)
  - 파생 가능한 값(가능하면 selector로 계산)
- store 작성 원칙:
  - action은 명시적 동사(`startSubmitting`, `markSuccess`)로 정의
  - selector 기반 구독으로 불필요한 리렌더 최소화
  - 테스트에서 `reset` 가능한 구조 유지
